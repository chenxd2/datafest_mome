...
# manipulate scale
s <- base3 %>%
  mutate(scaled_fatigue = round(StandardizeFatigue + 4)) %>%
...
```{r}
te <- lm(scaled_fatigue ~ scaled_sq + scaled_sl + scaled_desire + scaled_irr + scaled_sore + AcuteChronicRatio, data = s2)
  summary(te)
set.seed(100)
trainingRows <- sample(1:nrow(s), 0.7*nrow(s))
training <- s[trainingRows, ]
test <- s[-trainingRows, ]
